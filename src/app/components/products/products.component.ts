...
export class ProductsComponent implements OnInit {
  prodcutsState$: Observable<ProductsState> | null = null;
  readonly ProductStateEnum = ProductStateEnum;

  constructor(private store: Store<any>) {}
  // qu'on a le store le componenet fait un subscribe
  ngOnInit(): void {
    // quand il recoi l'ecoute que ce qui passe sur le store
    this.prodcutsState$ = this.store.pipe(map((state) => state.catalogueState));
  }
